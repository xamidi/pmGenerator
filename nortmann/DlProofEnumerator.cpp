...
map<uint32_t, map<uint32_t, uint64_t>>& DlProofEnumerator::iterationCounts_unfiltered() {
	static map<uint32_t, map<uint32_t, uint64_t>> _ = {
			...
			{ 35, {                                                                                                                                                               { 37, 2497890936 }, { 39, 8383579055 } } },
	};
	return _;
}
...
tbb::concurrent_unordered_set<uint64_t> DlProofEnumerator::_mpi_findRedundantConclusionsForProofsOfMaxLength(int mpi_rank, int mpi_size, const uint32_t maxLength, tbb::concurrent_unordered_map<string, string>& representativeProofs, const vector<string>& recentConclusionSequence, helper::ProgressData* const progressData, bool smoothProgress) {
	...
	if (progressData)
		progressData->setStartTime();
	auto worker = [&recentConclusionSequence, &indexDistribution, &split, &iterateFormulasOfStandardLengthUpTo](int mpi_rank, array<uint64_t, 2>& workload, vector<array<uint64_t, 2>>& reservableWorkloads, mutex& mtx_reservations, atomic<bool>& checkRequests, atomic<bool>& loading, size_t n, bool smoothProgress, bool isMainProc, atomic<uint64_t>& localCounter, condition_variable& cond, atomic<bool>& communicate, atomic<bool>& workerDone, tbb::concurrent_queue<uint64_t>& toErase, tbb::concurrent_unordered_set<uint64_t>& toErase_mainProc, helper::ProgressData* const progressData) {
