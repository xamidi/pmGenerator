...
void DlProofEnumerator::generateDProofRepresentativeFiles(uint32_t limit, uint32_t distributedNodesAmount, bool redundantSchemaRemoval, bool withConclusions) { // NOTE: More debug code & performance results available before https://github.com/deontic-logic/proof-tool/commit/45627054d14b6a1e08eb56eaafcf7cf202f2ab96 ; representation of formulas as tree structures before https://github.com/xamidi/pmGenerator/commit/2dd795c56b83f890689c40947df0ac96777f7ee4
	...
	tbb::parallel_for(representativeProofs.range(), [&maxLength, &progressData, &formulasByStandardLength, &toErase](tbb::concurrent_unordered_map<string, string>::range_type& range) {
		for (tbb::concurrent_unordered_map<string, string>::const_iterator it = range.begin(); it != range.end(); ++it) {
			...
			if (formula_sequenceLength == maxLength) {
				...
				tbb::parallel_for(formulasByStandardLength.range(), [&formulaLen, &redundant, &formula](tbb::concurrent_map<unsigned, tbb::concurrent_vector<const string*>>::range_type& range) {
