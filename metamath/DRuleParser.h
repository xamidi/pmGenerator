...
public:
	...
	static std::vector<std::string> unfoldAbstractDProof(const std::vector<std::string>& abstractDProof, const std::vector<AxiomInfo>* customAxioms = nullptr, bool targetEverything = false, std::vector<AxiomInfo>* filterForTheorems = nullptr, std::vector<AxiomInfo>* requiredIntermediateResults = nullptr, bool debug = false, std::size_t storeIntermediateUnfoldingLimit = SIZE_MAX, std::size_t limit = SIZE_MAX);
	static void compressAbstractDProof(std::vector<std::string>& retractedDProof, std::vector<std::shared_ptr<logic::DlFormula>>& abstractDProofConclusions, std::vector<std::string>& helperRules, std::vector<std::shared_ptr<logic::DlFormula>>& helperRulesConclusions, std::vector<std::size_t>& indexEvalSequence, std::vector<std::size_t>& targetIndices, const std::vector<AxiomInfo>* customAxioms = nullptr, bool targetEverything = false, std::vector<AxiomInfo>* filterForTheorems = nullptr, bool concurrentDRuleSearch = true, std::size_t modificationRange = 0, bool keepMaxRules = false, const std::string* vaultFile = nullptr, bool sureSaves = false, bool skipFirstPrep = false);
	static void removeDuplicateConclusionsFromAbstractDProof(std::vector<std::string>& retractedDProof, std::vector<std::shared_ptr<logic::DlFormula>>& abstractDProofConclusions, std::vector<std::string>& helperRules, std::vector<std::shared_ptr<logic::DlFormula>>& helperRulesConclusions, std::vector<std::size_t>& indexEvalSequence, std::vector<std::size_t>& targetIndices, const std::vector<AxiomInfo>* customAxioms = nullptr, bool targetEverything = false, std::vector<AxiomInfo>* filterForTheorems = nullptr, std::vector<AxiomInfo>* requiredIntermediateResults = nullptr, bool debug = false, bool silent = false);
	static std::vector<std::string> recombineAbstractDProof(const std::vector<std::string>& abstractDProof, std::vector<std::shared_ptr<logic::DlFormula>>& out_conclusions, const std::vector<AxiomInfo>* customAxioms = nullptr, bool targetEverything = false, std::vector<AxiomInfo>* filterForTheorems = nullptr, const std::vector<AxiomInfo>* conclusionsWithHelperProofs = nullptr, unsigned minUseAmountToCreateHelperProof = 2, std::vector<AxiomInfo>* requiredIntermediateResults = nullptr, bool debug = false, std::size_t maxLengthToKeepProof = SIZE_MAX, bool abstractProofStrings = true, std::size_t storeIntermediateUnfoldingLimit = SIZE_MAX, std::size_t limit = SIZE_MAX, bool removeDuplicateConclusions = false, bool compress = false, bool compress_concurrentDRuleSearch = true, std::size_t compress_modificationRange = 0, bool compress_keepMaxRules = false, const std::string* compress_vaultFile = nullptr, bool compress_sureSaves = false, bool compress_skipFirstPrep = false, bool silent = false);
private:
