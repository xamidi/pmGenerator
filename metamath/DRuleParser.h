...
public:
	...
	static void compressAbstractDProof(std::vector<std::string>& retractedDProof, std::vector<std::shared_ptr<logic::DlFormula>>& abstractDProofConclusions, std::vector<std::string>& helperRules, std::vector<std::shared_ptr<logic::DlFormula>>& helperRulesConclusions, std::vector<std::size_t>& indexEvalSequence, std::vector<std::size_t>& targetIndices, const std::vector<AxiomInfo>* customAxioms = nullptr, bool targetEverything = false, std::vector<AxiomInfo>* filterForTheorems = nullptr, std::size_t modificationRange = 0, bool keepMaxRules = false, const std::string* vaultFile = nullptr, bool sureSaves = false, bool skipFirstPrep = false, bool babySteps = false);
