...
namespace {
void recurse_primitiveSequenceOfFormula(const shared_ptr<DlFormula>& formula, vector<string>& result) {
	if (formula->getChildren().empty()) {
		const string& value = formula->getValue()->value;
		if (value != DlCore::terminalStr_bot() && value != DlCore::terminalStr_top())
		...
		for (const shared_ptr<DlFormula>& subformula : formula->getChildren())
			recurse_primitiveSequenceOfFormula(subformula, result);
}
}
vector<string> DlCore::primitiveSequenceOfFormula(const shared_ptr<DlFormula>& formula) {
	vector<string> result;
	recurse_primitiveSequenceOfFormula(formula, result);
	return result;
}
...
namespace {
string recurse_toPolishNotation(const shared_ptr<DlFormula>& node, const vector<string>& sequenceOfVarNames, const unordered_map<string, string>& operatorNames, map<string, string>& operatorTranslations, map<string, string>& variableTranslations, size_t& nextVariableIndex) {
	...
	for (size_t i = 0; i < node->getChildren().size(); i++)
		str += recurse_toPolishNotation(node->getChildren()[i], sequenceOfVarNames, operatorNames, operatorTranslations, variableTranslations, nextVariableIndex);
	return str;
}
}
string DlCore::toPolishNotation(const shared_ptr<DlFormula>& f, bool prioritizeBochenski, const map<string, string>* customOperatorTranslation, const map<string, string>* customVariableTranslation, const vector<string>& sequenceOfVarNames) {
	...
	return recurse_toPolishNotation(f, sequenceOfVarNames, operatorNames, operatorTranslations, variableTranslations, nextVariableIndex);
}
...
namespace {
string recurse_toPolishNotation_numVars(const shared_ptr<DlFormula>& node, bool& startsWithVar, bool& endsWithVar, const unordered_map<string, string>& operatorNames, unordered_map<string, string>& variableTranslation) {
	auto valToString = [&](const string& s, bool& isVar) -> string {
		// 1. Operator names
		unordered_map<string, string>::const_iterator searchResult = operatorNames.find(s);
		if (searchResult != operatorNames.end()) {
	...
	for (size_t i = 0; i < node->getChildren().size(); i++) {
		bool childStartsWithVar, childEndsWithVar;
		string tmp = recurse_toPolishNotation_numVars(node->getChildren()[i], childStartsWithVar, childEndsWithVar, operatorNames, variableTranslation);
		str += prevEndsWithVar && childStartsWithVar ? "." + tmp : tmp;
		prevEndsWithVar = childEndsWithVar;
	}
	endsWithVar = prevEndsWithVar;
...
string DlCore::toPolishNotation_numVars(const shared_ptr<DlFormula>& f, bool prioritizeBochenski) {
	...
	bool x, y;
	return recurse_toPolishNotation_numVars(f, x, y, operatorNames, variableTranslation);
}
